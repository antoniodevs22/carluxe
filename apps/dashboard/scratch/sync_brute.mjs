...
async function seed() {
  console.log('--- STARTING CLEAN SEED (V3) ---')

  await supabase.from('ordens_servico').delete().neq('status', 'cancelado')
  await supabase.from('agendamentos').delete().neq('status', 'cancelado')
  ...
  for (const item of osData) {
    ...
    const { error: veiculoErr } = await supabase.from('veiculos').insert({
      id: vehicleId,
      cliente_id: userId,
      marca: item.car.split(' ')[0],
      modelo: item.car,
      placa: item.plate,
      ano: 2024,
      cor: 'Preto',
      porte: 'medio' // Minúsculo, sem acento
    })
    
    if (!veiculoErr) {
       await supabase.from('vehicles').insert({ id: vehicleId, customerId: userId, model: item.car, plate: item.plate, brand: item.car.split(' ')[0], color: 'Preto', year: '2024' })
       ...
       console.error(`Veiculo Error for ${item.car}:`, veiculoErr.message)
    }
  }
  ...
  for (const a of agenda) {
    ...
    await supabase.from('veiculos').insert({ id: vehicleId, cliente_id: userId, modelo: a.car, placa: a.plate, porte: 'medio' })
